...
// require_once("./sites/global/sqlconf.php")
...
$drug_id         = $_REQUEST['drug_id'] ?? null;
$prescription_id = $_REQUEST['prescription'] ?? null;
$quantity        = $_REQUEST['quantity'] ?? null;
$fee             = $_REQUEST['fee'] ?? null;
$user            = $_SESSION['authUser'] ?? null;
$encounter       = $_SESSION['encounter'] ?? null;

// Check if $sale_id is null or not set
if ($sale_id === null) {
    echo "<div id='message'>Medication Saved and Patient Billed! $drug_id, No Of Tablets: $quantity. </div>"; // Handle the case when 'sale_id' is not set
}
...
//
 ...
 if ( !$sale_id) {
  ...
  //
    ...
    //this uncommenting causes double reduction of meds instead,
